...
function renderCustomConfigs(): void {
  ...
  container.innerHTML = customConfigs
    .map(
      (config, index) => `
...
function openAddCustomModal(): void {
  ...
  const stdioRadio = document.querySelector<HTMLInputElement>(
    'input[name="connectionType"][value="stdio"]'
  );
...
function loadConfigToForm(config: PresetConfig): void {
  ...
  const radio = document.querySelector<HTMLInputElement>(
    `input[name="connectionType"][value="${type}"]`
  );
  if (radio) {
    radio.checked = true;
  }
  ...
  if (type === 'stdio') {
    ...
    if (httpHeaders)
      httpHeaders.value = config.headers ? JSON.stringify(config.headers, null, 2) : '';
  } else if (type === 'sse') {
    if (sseUrl) sseUrl.value = config.url || '';
    if (sseHeaders)
      sseHeaders.value = config.headers ? JSON.stringify(config.headers, null, 2) : '';
  }
...
function buildConfigFromForm(): PresetConfig {
  ...
  try {
    ...
    throw new Error(
      error.message.includes('JSON') ? 'JSON 格式錯誤：' + error.message : error.message
    );
  }
